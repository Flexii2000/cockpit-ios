...
final class Access {
    ...
    init() {
        // Vor dem Lesen: sonst laege alles noch in der alten Gruppe und die
        // App fragte nach Token, die laengst da sind.
        Self.migrateToSharedGroup()
    ...
    /// Holt die Token aus der Gruppe der frueheren einen App in die geteilte.
    ///
    /// Einmalig, und nur dort erfolgreich, wo die alte Gruppe erreichbar ist:
    /// in Healthy, die die Bundle-ID behalten hat. Vault und Fokus lesen ins
    /// Leere und tun nichts - sie finden die Eintraege, sobald Healthy einmal
    /// gestartet war. Das Noten-Passwort wandert **nicht** mit: es liegt
    /// hinter Face ID, und eine Abfrage beim Start einer App, die es gar
    /// nicht braucht, waere Unsinn. Vault fragt es einmal neu ab.
    static func migrateToSharedGroup() {
        let defaults = UserDefaults.standard
        guard !defaults.bool(forKey: sharedGroupKey) else { return }
        var moved = 0
        for key in [privateTokenKey, weightTokenKey, gradesTokenKey, Keychain.gradesUserKey]
        where Keychain.read(key) == nil {
            if let value = Keychain.read(key, group: Keychain.legacyAccessGroup) {
                Keychain.save(value, for: key)
                moved += 1
            }
        }
        // Erst als erledigt merken, wenn die alte Gruppe ueberhaupt lesbar
        // war - sonst uebersprang Vault die Wanderung fuer immer, obwohl
        // Healthy sie spaeter noch gebraucht haette. Ein Merker je App, also
        // je Container, das passt.
        if moved > 0 || Keychain.read(privateTokenKey, group: Keychain.legacyAccessGroup) != nil
            || Keychain.read(privateTokenKey) != nil {
            defaults.set(true, forKey: sharedGroupKey)
        }
    }
    ...
    func store(privateToken: String) async {
    ...
    func store(weightToken: String) async {
