...
final class WeightModelTests: XCTestCase {
    ...
    func testDecodesSummary() throws {
        ...
        {"date":"2026-09-01","current":83.2,"avg7":83.5,"avg14":83.8,"avg30":84.4,
         ...
         "corridorReachedOn":"2026-07-14","residual7":0.43,"residual7Days":7}
        """.utf8)
        ...
        XCTAssertEqual(summary.residual7, 0.43)
        XCTAssertEqual(summary.residual7Days, 7)
    }

    /// Ein Dienst von vor der Kachel kennt `residual7` nicht - die Summary muss
    /// trotzdem laden, sonst waere der ganze Tab leer.
    func testSummaryLoadsWithoutSevenDayResidual() throws {
        let json = Data("""
        {"date":"2026-09-01","current":83.2,"avg7":83.5,"avg14":83.8,"avg30":84.4,
         "target":82.7,"targetDate":"2026-12-20","goalWeight":82.0,"startWeight":92.0,
         "recordingStart":"2025-01-05","corridorLower":80.5,"corridorUpper":83.5,
...
final class WeightWidgetTests: XCTestCase {
    ...
    func testSevenDayResidualShowsSignedMeanWithDiffTones() {
        let slightly = summary(corridorReached: false, residual7: 0.43, residual7Days: 7)
        XCTAssertEqual(WeightWidget.residual7.value(slightly), "+0.4 kg")
        XCTAssertEqual(WeightWidget.residual7.tone(slightly), .warn)
        XCTAssertNil(WeightWidget.residual7.note(slightly))

        XCTAssertEqual(WeightWidget.residual7.tone(summary(corridorReached: false, residual7: 2.1, residual7Days: 7)), .bad)
        let below = summary(corridorReached: false, residual7: -0.8, residual7Days: 7)
        XCTAssertEqual(WeightWidget.residual7.value(below), "-0.8 kg")
        XCTAssertEqual(WeightWidget.residual7.tone(below), .good)
    }

    /// Beim Halten ist ein Wochenmittel innerhalb der halben Korridorbreite
    /// der Normalfall - erst darueber wird es rot.
    func testSevenDayResidualToleratesTheCorridorOnceReached() {
        XCTAssertEqual(WeightWidget.residual7.tone(summary(residual7: 1.2, residual7Days: 7)), .good)
        XCTAssertEqual(WeightWidget.residual7.tone(summary(residual7: 1.6, residual7Days: 7)), .bad)
    }

    /// Fehlen Tage, sagt es die Kachel - das Mittel ist dann schmaler, als
    /// sein Name verspricht.
    func testSevenDayResidualNotesMissingDays() {
        XCTAssertEqual(WeightWidget.residual7.note(summary(residual7: 0.2, residual7Days: 5)), "5 von 7 Tagen")
        XCTAssertNil(WeightWidget.residual7.note(summary(residual7: 0.2, residual7Days: 7)))
        XCTAssertNil(WeightWidget.residual7.note(summary(residual7: nil, residual7Days: 0)))
        XCTAssertEqual(WeightWidget.residual7.value(summary(residual7: nil, residual7Days: 0)), "–")
    }
